...
plot_dat <- d |> 
    ...
    mutate(
        ...
        text = str_c("<b>Tekjutíund: ", tiundarhluti, "</b>", "\n",
                     ...
                     number(value * 1e6 / (fjoldi * 12), suffix = " kr", big.mark = ".", decimal.mark = ","), "\n",
                     "Breyting (síðan 1997): ", 
                      ifelse(index >= 1, "+", ""),
...
## Ráðstöfunartekjur sem hlutfall af tekjum
...
plot_dat <- d |> 
    filter(tiundarbreyta == "Heildartekjur",
           name %in% c("Tekjur alls", "Ráðstöfunartekjur (Tekjur - Skattar)")) |> 
    collect() |> 
    ...
    janitor::clean_names() |> 
    mutate(
        value = radstofunartekjur_tekjur_skattar / tekjur_alls,
        index = value / value[ar == min(ar)],
        text = str_c("<b>Tekjutíund: ", tiundarhluti, "</b>", "\n",
                     "Ár: ", ar, "\n",
                     "Ráðstöfunartekjur % af heildartekjum: ", 
...
p <- plot_dat |> 
    ...
    labs(x = NULL,
         y = NULL,
         title = "Breyting á ráðstöfunartekjum sem hlutfall af heildartekjum miðað við 1997")
...
p <- plot_dat |> 
    ...
    scale_y_log10(
        labels = function(x) percent(x - 1),
        breaks = c(seq(0.9, 1.1, by = 0.05), range(plot_dat$index))
    ) +
    scale_colour_distiller(type = "div", palette = "RdYlBu", direction = 1) +
    theme_tufte() +
    ...
    labs(x = NULL,
         ...
         colour = "Tekjutíund",
         title = "Breyting á ráðstöfunartekjum sem hlutfall af heildartekjum miðað við 1997",
         subtitle = "Ráðstöfunartekjur hafa hækkar hægar en heildartekjur í öllum tekjutíundum nema þeirri efstu")
