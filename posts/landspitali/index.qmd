...
## Hlutfall lausra rýma

Á hverjum tímapunkti er mikilvægt að ákveðið hlutfall legurýma sé laust, bæði til að bregðast við slysum, en líka til að geta fært sjúklinga á milli deilda. Við getum reiknað meðaltal hlutfalls lausra rýma með því að reikna meðalfjölda inniliggjandi sjúklinga og deila í með meðalfjölda legurýma yfir árið.
...
plot_dat <- d |> 
    mutate(nyting = medalfjoldi_sjukl/fjoldi_rum,
           laust_plass = 1 - nyting) |> 
    drop_na(laust_plass, ar) |> 
    mutate(text = str_c("Ár: ", ar, "\n",
                        "Meðalfjöldi rúma: ", fjoldi_rum, "\n",
                        "Meðalfjöldi inniliggjandi: ", medalfjoldi_sjukl, "\n",
                        "Hlutfall lausra rúma: ", percent(laust_plass, accuracy = 0.1, 
                                                          big.mark = ".", decimal.mark = ",")))


p <- plot_dat |> 
    ggplot(aes(ar, laust_plass)) +
    geom_hline(yintercept = 0.15, lty = 2, alpha= 0.5) +
    geom_area(aes(text = text, group = "none"), alpha = 0.5, col = "black") +
    scale_x_continuous(breaks = 2008:2022,
                       labels = c(2008:2021, "2022\n(Jan-Mars)"),
                       expand = expansion()) +
    ...
    labs(x = NULL,
         y = NULL,
         title = "Hlutfall lausra legurýma á Landspítala (2008 - 2022)")
...
Hér má sjá fjölda og hlutföll stöðugilda fyrir fleiri stéttarfélög. Þar sem Sameyki varð ekki til fyrr en 2019 lagði ég saman fjölda stöðugilda hjá viðeigandi stéttarfélögum fyrir þann. 
